...
**手动账户对照**：`manual-log` 记录（已播种：近三月 N/A vs 上证 -3.53%）；用户每月报一个数。
...
1. **券商一个电话办三件事**: 开 miniQMT(门槛以券商为准,券商AppApp可自助) + 佣金谈万一免五
   + 确认程序化报备(报"中低频轮动")。回来带佣金数字 → E49 当晚出判决;
   权限下来 → 我造 QMT 执行器(dry-run 默认, 真钱开关锁在"模拟≥30天+过门槛"后)
2. **IB Gateway 每日重登**（或配自动重启）——futures_ibkr 唯一依赖
3. **每月 manual-log 一个数字**（手动账户收益%, 对照实验的燃料）
4. OKX 实盘维持搁置（用户虚拟币偏好，不催）

## 七、我方待办（下会话优先级）

1. **E65 转债下修博弈**: 预注册未冻结、数据已探通(bond_cb_redeem_jsl 322行/
   bond_cb_adj_logs_jsl 逐券下修史)——A股最后一块免费矿
2. QMT 执行器（等用户权限）；分钟线 ¥2000 决策（等 E65+佣金落地一起议）
...
explain / weekly（含统计+衰减+手动对照+llm成本）/ health / paper-ab（统计裁决器）/
paper-report（luck检验）/ manual-log / revalidate.py(月度) / universe_score.py(季度)
