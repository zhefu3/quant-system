...
十本纸面账并行（2 验证 + 3 平行/演示 + 5 观察），五资产类+四信号源；执行层 OKX 就绪、
...
- **E70 预注册已冻结未实现**: llm_us 美股 LLM 委员会观察账本(宇宙名单冻结附录
  待做, 实现排队)。E72(A股执行时点覆盖层)/E73(极端资金费反转)预注册排队。
...
- **采集层**: 五条自制 PIT 流(转债事件/funding/OI/涨停池/龙虎榜), funding+OI 小时级;
  **funding 历史回填**(data.binance.vision, E56封锁被解封)5/11 完成, 中断则重跑
  research/backfill_funding.py 续传。
...
4. **实现队列**: E70 llm_us 账本 → E72/E73 预注册文本 → 再平衡分桶设计(入选#1)
