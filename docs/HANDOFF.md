...
2. **IBC 激活两步**（自动重登已装好, 见 ops-runbook）: ~/ibc/config.ini 填两行
   凭据（paper 账户则 TradingMode 同改）→ 退出手动 Gateway → launchctl bootstrap。
   激活前仍需每日手动重登
...
tca（实付vs假设成本）/ paper-ab（统计裁决器）/ paper-report（luck检验）/
